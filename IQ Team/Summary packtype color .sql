...
cbp.BP,CLPT.LPT ,CPT.PT,CCLR.CLR,npd.NPD,WP.WP,EB.EB,RL.RL,COTC.COTC
from  
-- BP / Lines
(
	select RS_CODE as 'DT CODE' ,UP_code as 'POPCODE',COunt(UP_code) BP from
	SUGGESTED_ORDER a 
	where MOC_NO ='07' and MOC_YEAR ='2019'
	group by RS_CODE,UP_code
	) CBP 
-- count local_packtype
		left Join (
		select a.[DT CODE],a.[POPCODE],COunt(a.local_packtype) LPT from
		(select distinct RS_CODE as 'DT CODE' ,UP_code as 'POPCODE', LOCAL_PACKTYPE from 
		SUGGESTED_ORDER
		where MOC_NO ='07' and MOC_YEAR ='2019') a 
		group by a.[DT CODE],a.[POPCODE]
		) CLPT on CBP.[POPCODE] = CLPT.[POPCODE]
-- count packtype
	left join (
	select a.[DT CODE],a.[POPCODE],COunt(a.PACKTYPE) PT from
	(select distinct RS_CODE as 'DT CODE' ,UP_code as 'POPCODE', PACKTYPE from 
	SUGGESTED_ORDER
	where MOC_NO ='07' and MOC_YEAR ='2019') a 
	group by a.[DT CODE],a.[POPCODE]
	) CPT on CBP.[POPCODE] = CPT.[POPCODE]
-- count COLOR
		left join (
		select a.[DT CODE],a.[POPCODE],COunt(a.COLOR) CLR from
		(select distinct RS_CODE as 'DT CODE' ,UP_code as 'POPCODE', COLOR from 
		SUGGESTED_ORDER
		where MOC_NO ='07' and MOC_YEAR ='2019') a 
		group by a.[DT CODE],a.[POPCODE]
		) CCLR on CBP.[POPCODE] = CCLR.[POPCODE]
-- join pop
...
select RS_CODE as 'DT CODE' ,UP_code as 'POPCODE',COunt(UP_code) NPD from
	SUGGESTED_ORDER a 
	where MOC_NO ='07' and MOC_YEAR ='2019' and PACKTYPE ='5'
	group by RS_CODE,UP_code
...
select RS_CODE as 'DT CODE' ,UP_code as 'POPCODE',COunt(UP_code) WP from
	SUGGESTED_ORDER a 
	where MOC_NO ='07' and MOC_YEAR ='2019' and PACKTYPE ='2'
	group by RS_CODE,UP_code
...
select RS_CODE as 'DT CODE' ,UP_code as 'POPCODE',COunt(UP_code) EB from
	SUGGESTED_ORDER a 
	where MOC_NO ='07' and MOC_YEAR ='2019' and PACKTYPE ='1'
	group by RS_CODE,UP_code
...
select RS_CODE as 'DT CODE' ,UP_code as 'POPCODE',COunt(UP_code) RL from
	SUGGESTED_ORDER a 
	where MOC_NO ='07' and MOC_YEAR ='2019' and Color ='1'
	group by RS_CODE,UP_code
...
select RS_CODE as 'DT CODE' ,UP_code as 'POPCODE',COunt(UP_code) COTC from
	SUGGESTED_ORDER a 
	where MOC_NO ='07' and MOC_YEAR ='2019' and LOCAL_PACKTYPE ='1'
	group by RS_CODE,UP_code
